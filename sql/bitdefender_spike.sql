-- Zoom in on the Bitdefender spike of August 2025.
--
-- Usage (from the repo root):
--   duckdb -readonly thunderbird.duckdb -f sql/bitdefender_spike.sql
--
...
--
-- Writes two files:
--   output/bitdefender_daily.csv  -- daily counts across a Jul-Sep window
--   output/bitdefender_spike_threads.csv -- the individual threads
--
...
COPY (
    ...
    GROUP BY 1
    ORDER BY 1
) TO 'output/bitdefender_daily.csv' (HEADER);
...
COPY (
    ...
    ORDER BY created
) TO 'output/bitdefender_spike_threads.csv' (HEADER);

SELECT day, all_questions, bitdefender_questions, pct_bitdefender
FROM read_csv('output/bitdefender_daily.csv')
