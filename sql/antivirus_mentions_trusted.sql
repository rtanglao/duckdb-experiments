...
-- counting only answers that pass the trusted filter. Derived from
-- sql/antivirus_mentions.sql; the sole difference is `answers` ->
-- `answers_trusted` in the thread temp table. See sql/trusted_contributors.sql.
--
-- Usage (from the repo root):
--   duckdb -readonly thunderbird.duckdb -f sql/antivirus_mentions_trusted.sql
--   uv run sql/plot_antivirus.py --trusted
--
-- Writes output/antivirus_quarterly_trusted.csv, the twin of the unfiltered
-- output/antivirus_quarterly.csv. Question text is untouched, so a thread only
-- leaves the count when the vendor was named solely in a dropped answer.
--
...
) TO 'output/antivirus_quarterly_trusted.csv' (HEADER);

-- Totals, for the record.
SELECT series, sum(threads) AS threads
FROM read_csv('output/antivirus_quarterly_trusted.csv')
GROUP BY series
